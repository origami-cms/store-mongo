...
export interface toJSONHidden { hidden?: boolean; }
...
export default class MongoModel extends Model {
    ...
    protected async _find(
        ...
        this._parseQuery(query as MongoDocument);
        let func = this._mModel.find(query);
        func = await this._populateQuery(func, options);
    ...
    protected async _update(
        ...
        const toUpdate = await this._mModel.find(this._parseQuery(query as MongoDocument));
        ...
        Object.entries(rules).forEach(([rule, [r, func]]) => {
            if (r.test(e.message)) {
                this._validationError(
                    ...
                    rule,
                    fieldRegex.exec(e.message)![1] as ValidationMessages
                );
            }
        });
    ...
    private async _populateQuery(func: mongoose.DocumentQuery<any, any>, options: any) {
        let f = func;
        ...
        if (options && options.include) {
            ...
            if (include.length) f = await f.exec();
        }

        return func;
    }

    private _parseQuery(query: MongoDocument) {
